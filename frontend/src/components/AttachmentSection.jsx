...
//                 <a
//                   // href={att.url}
//                   href={`/api/tasks/${taskId}/attachments/${att._id}/open`}
//                   // href={
//                   //   att.url.includes('/image/upload/')
//                   //     ? att.url.replace('/image/upload/', '/image/upload/fl_inline/')
//                   //     : att.url
//                   // }
//                   target="_blank"
//                   rel="noreferrer"
...
//                 {/* DOWNLOAD */}
//                 <a
//                   href={`/api/tasks/${taskId}/attachments/${att._id}/download`}
//                   target="_blank"
//                   rel="noreferrer"
//                   className="btn btn-ghost btn-sm btn-icon"
//                   title="Download"
...
const AttachmentSection = ({ taskId, attachments = [], onUpdate, isAdmin }) => {
  ...
  const handleOpen = async (att) => {
    try {
      ...
      const res = await fetch(`${BASE}/tasks/${taskId}/attachments/${att._id}/open`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (!res.ok) throw new Error('Failed');
      const buffer = await res.arrayBuffer();
      const blob = new Blob([buffer], { type: att.mimetype || 'application/pdf' });
      const blobUrl = URL.createObjectURL(blob);
      const newTab = window.open('', '_blank');
      newTab.document.write(`
      <!DOCTYPE html>
      <html>
        <head><title>${att.originalName}</title></head>
  ...
  const handleDownload = async (att) => {
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${BASE}/tasks/${taskId}/attachments/${att._id}/download`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (!res.ok) throw new Error('Failed');
      const buffer = await res.arrayBuffer();
