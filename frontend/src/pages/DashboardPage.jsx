  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontSize: 'clamp(18px, 4vw, 24px)', fontWeight: 700, marginBottom: 4 }}>
          ...
          {stats?.total > 0 ? (
            <ResponsiveContainer width="100%" height={chartHeight}>
              <PieChart>
                ...
                <Legend formatter={(v) => (
                  <span style={{ color: 'var(--text-secondary)', textTransform: 'capitalize', fontSize: 12 }}>
                    {v.replace('-', ' ')}
                  </span>
                )} />
              </PieChart>
      ...
      <div className="card">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 12 }}>
          {[
            ...
            <div key={key} style={{ background: 'var(--bg-hover)', borderRadius: 'var(--radius)', padding: isMobile ? 12 : 16, textAlign: 'center' }}>
              <div style={{ fontSize: 20, color, marginBottom: 4 }}>{icon}</div>
              <div style={{ fontSize: isMobile ? 18 : 22, fontWeight: 700, color }}>{stats?.byStatus[key] || 0}</div>
